...
def cook(info: dict):
    ...
    async def run():
        logg.info(f"[cook] {order.id} coming right up!", header=True)
        ...
        logg.info(f"[cook] {order.id} done cooking!", header=True)
        cook_is_tired = random() > 0.7
        if cook_is_tired:
            logg.info(f"[cook] taking a short break...")
    ...
    try:
        ...
        logg.error(f"[cook] {order.id} got messed up!")
